...
author: "Toni Monleon & Nicolas Ayala. BIOST3. UNIV BARCELONA"
date: "20/5/2020"
...
```
Standar rate vs Density.
```{r}
plot (model_risk$standar_rate_10k ~ model_risk$density, data=model_risk, xlab="Density inhab. /km2", ylab="Standar rate ")
```
...
Standar rate and NOx reduction (%) in March 2020 compared to 2017-2019
```{r}
plot(model_risk$standar_rate_10k ~ model_risk$NOX_reduction_march2020, data=model_risk, col="red", xlab="NOx reduction (%)", ylab="Standar rate COVID-19", xlim=c(0, 60))
```


Standar rate and NOx reduction (%) in April 2020 compared to 2017-2019
```{r}
plot(model_risk$standar_rate_10k ~ model_risk$NOX_reduction_april2020, data=model_risk, col="red", xlab="Nox reduction", ylab="standar_rate_10k COVID19", xlim=c(0, 80))
```

Standar rate and NOx reduction (%) in May 2020 compared to 2017-2019
```{r}
plot(model_risk$standar_rate_10k ~ model_risk$NOX_reduction_may2020, data=model_risk, col="red", xlab="Nox reduction", ylab="standar_rate_10k COVID19", xlim=c(0, 110))
```

Standar rate of COVID19 and  NOx reduction (%) in May 2020 compared to May 2017-2019
```{r}
plot(model_risk$standar_rate_10k ~ model_risk$NOX_reduction_may2020, data=model_risk, col="red", xlab="NOx reduction in may 2020", ylab="standar_rate_10k COVID19", xlim=c(0, 120))
```
...
-standar_rate_10k: Standar rate (10.000 inhab) of COVID-19
...
Standar rate and population + density + surface
```{r}
# taxa_ESTANDARITZADA
res2<-lm(standar_rate_10k ~ surface_km2 + population , data=model_risk)
summary(res2)
...
Standar rate of COVID-19 and density
```{r}
# taxa_ESTANDARITZADA
res2<-lm(standar_rate_10k ~ density , data=model_risk)
summary(res2)

```

**Relative Ecologic Risk Morguenson 1982 ** 
Standar rate of COVID-19 compared to Density
```{r}
# Standar rate of COVID-19
res2<-lm(standar_rate_10k~ density , data=model_risk)
#summary(res2)
...
final.RRE<-RRE(X=model_risk$density ,Y=model_risk$standar_rate_10k,num.itera=1000,nsize=200)
...
Standar rate of COVID19 and pollution reduction
```{r}
#change data format
...
standar_rate_10k vs pullution reduction (March 2020) 
**Relative Ecologic Risk Morguenson 1982 ** 
```{r}
res1<-lm(standar_rate_10k ~ NOX_reduction_march2020 , data=model_risk)
summary(res1)
...
final.RRE<-RRE(X=model_risk$NOX_reduction_march2020,Y=model_risk$standar_rate_10k,
               num.itera=1000,nsize=200)

final.RRE
```

Standar rate of COVID19 and pollution reduction (April 2020) and **Ecological Relative Risk Morguenson 1982 **:
```{r}
...
#using resamplen to compute CI95
final.RRE<-RRE(X=model_risk$NOX_reduction_april2020,Y=model_risk$standar_rate_10k,
               num.itera=1000,nsize=200)

final.RRE
```
Standar rate of COVID19 and pollution reduction (May 2020) and **Ecological Relative Risk Morguenson 1982 **:
```{r}
...
#using resamplen to compute CI95
final.RRE<-RRE(X=model_risk$NOX_reduction_may2020,Y=model_risk$standar_rate_10k,
...
Standar rate of COVID19 vs Pollution reduction (%) using March, April and May:
```{r}
...
## ###################################################################################
## UNIVARIATE MODELS OF COVID-19 RISK ---Y=standar_rate_10k: --------------
## ###################################################################################
The level of aggregation in this case is: municipality/portion present in the system of the "Generalitat de Catalunya"
Below are multiple regression models with various regressors (density, population):
# COVID-19 risk modeling (Y= standar_rate_10k, X=  density)

Standar Rate of COVID-19 (standar_rate_10k):
```{r}
res1<-lm(standar_rate_10k ~ density , data=model_risk)
summary(res1)
print("Relative Ecologic Risk---------------------------------:")
(res1$coefficients[1] + res1$coefficients[2])/res1$coefficients[1] 
```

# COVID-19 risk modeling  (Y= standar_rate_10k, X=  income_euros)
Familiar Income (RFDB (milers d'euros)) 	
```{r}
res1<-lm(standar_rate_10k ~ income_euros , data=model_risk)
summary(res1)
print("Relative Ecologic Risk---------------------------------:")
(res1$coefficients[1] + res1$coefficients[2])/res1$coefficients[1] 
```

# COVID-19 risk modeling (Y= standar_rate_10k, X=  income_euros_inhab)
Familiar Income per inhabitant (RFDB per habitant (milers d'euros))	
```{r}
res1<-lm(standar_rate_10k ~ income_euros_inhab , data=model_risk)
print(res1)
summary(res1)
...
final.RRE<-RRE(X=model_risk$income_euros_inhab,Y=model_risk$standar_rate_10k,
               num.itera=1000,nsize=200)
final.RRE
```
...
```{r}
res1<-lm(standar_rate_10k ~ income_index_100 , data=model_risk)
summary(res1)
print("Relative Ecologic Risk---------------------------------:")
(res1$coefficients[1] + res1$coefficients[2])/res1$coefficients[1] 
```

# COVID-19 risk modeling  (Y= standar_rate_10k, X=  pollution reduction in April 2020)
...
# COVID-19 risk modeling  (Y= standar_rate_10k, X=  pollution reduction in May 2020)
...
## ###################################################################################
## MULTIVARIATE MODELS OF COVID-19 RISK ---Y=standar_rate_10k: -----------------------
## ###################################################################################

# COVID-19 risk modeling with interaction (Y= standar_rate_10k, X= pollution reduction(%) in March, April, May, income_euros_inhab, density)
